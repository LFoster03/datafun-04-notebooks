"""app_foster_tips_dataset.py - Project script.

Author: Lindsay Fostet
Date: 2026-04
...
Data Source:
- tips dataset - contains information about restaurant tips, including total bill, tip amount, day of the week, and more.
- Available via Seaborn

Assumptions:
- The data contains columns like:
  total_bill, tip, day, sex, smoker, time

Terminal command to run this file from the root project folder:

uv run python -m datafun.app_foster_tips_dataset
...
def make_plots(df_clean: pd.DataFrame) -> None:
    ...
    scatter_plt.set_title("Total Bill vs Tip")
    ...
    box_plt.set_title("Total Bill by day")
...
def summarize(df: pd.DataFrame, df_clean: pd.DataFrame) -> None:
    ...
    # Get the unique values in the grouping column (e.g. day)
    ...
    avg_tip_by_day = df_clean.groupby("day")["tip"].mean()

    best_day = avg_tip_by_day.idxmax()
    best_tip = avg_tip_by_day.max()

    LOG.info(f"Day with highest average tip: {best_day} ({best_tip:.2f} $)")

    # Additional technical modification: tip differences by smoker status
    avg_tip_by_smoker = df_clean.groupby("smoker")["tip"].mean()

    LOG.info("Average tip by smoker status:")
    LOG.info(f"\n{avg_tip_by_smoker}")

    LOG.info(
        "Observation: tips vary across days, suggesting behavioral or social patterns."
    )
    LOG.info(
        "  best_day and best_tip variables capture the day with the highest average tip and the amount."
    )

    LOG.info("Suggested next step:")
    LOG.info("  Compare tips by smoker status or time of day (lunch vs dinner)")
